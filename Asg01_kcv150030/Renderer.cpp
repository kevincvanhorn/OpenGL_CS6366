...
ObjectManager* Renderer::m_objectManager = new ObjectManager(m_animation);
...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addVariable("Rotate Angle a:", m_objectManager->RotAngleA)->setSpinnable(true);
	gui_1->addVariable("Rotate Angle b:", m_objectManager->RotAngleB)->setSpinnable(true);
	gui_1->addButton("Rotate Local X", []() {
		m_objectManager->rotateLocalX();
		});
	gui_1->addButton("Rotate Global Y", []() {
		m_objectManager->rotateGlobalY();
		});
	gui_1->addButton("Reset Model", []() {
		m_objectManager->resetModel();
		});
