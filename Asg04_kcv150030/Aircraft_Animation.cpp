...
void Aircraft_Animation::update(float delta_time)
{
	...
	if (bAircraftMoving) {
		...
		// Get the point interp:
		m_animation_curve->GetIndexPointFromDist(im1, i, realDist, interp);

		glm::quat rot1 = m_animation_curve->control_points_quaternion[im1];
		glm::quat rot2 = m_animation_curve->control_points_quaternion[i];
		glm::quat newRot = glm::slerp(rot1, rot2, interp);


		printf("%f %i\n",interp, im1);
		m_model_mat = glm::mat4(1.0f);
		glm::mat4 rotMatrix = glm::toMat4(newRot);
		m_model_mat = glm::translate(m_model_mat, newPos) * rotMatrix;
...
void Aircraft_Animation::reset()
{
	...
	{
		m_model_mat = glm::translate(m_model_mat, m_animation_curve->control_points_pos[0]) * glm::toMat4(m_animation_curve->control_points_quaternion[0]);
	}
}
